...
# Change directory
setwd("/Users/rajesh/Dropbox/Documents/CAN_IMG/01_PMCRC/Projects/00_Projects/01_FLOMICS/00_Methylation_Analysis/18_DGE_Analysis/DESeq2/")
...
dds$condition <- relevel(dds$condition, ref = "iFL")
dds$condition
...
resLFC <- lfcShrink(dds, coef = "condition_aFL_vs_iFL", type = "apeglm")
res <- results(dds, name = "condition_aFL_vs_iFL", independentFiltering = TRUE)
...
par(mfrow = c(1, 2))
DESeq2::plotMA(resLFC, main="Shrinkage of LFCs", ylim=c(-4,4))
DESeq2::plotMA(res, main="No shrinkage of LFCs", ylim=c(-4,4))
dev.off()
...
write.table(as.data.frame(res), file = "DEG_aFL_vs_iFL_All.tsv", quote = FALSE, sep = "\t")
...
summary(res05)
write.table(as.data.frame(res05), file = "DEG_aFL_vs_iFL_res05.tsv", quote = FALSE, sep = "\t")

# up-regulated genes
res05up <- subset(res05, log2FoldChange >= 0.1)
summary(res05up)
write.table(as.data.frame(res05up), file = "DEG_aFL_vs_iFL_res05up.tsv", quote = FALSE, sep = "\t")

# down-regulated genes
res05down <- subset(res05, log2FoldChange <= -0.1)
summary(res05down)
write.table(as.data.frame(res05down), file = "DEG_aFL_vs_iFL_res05down.tsv", quote = FALSE, sep = "\t")

# Volcano Plot
# png("volcano-plot.png", units="px", width=4500, height=4000, res=600)
maxY <- max(-log10(res$padj), na.rm = TRUE)
