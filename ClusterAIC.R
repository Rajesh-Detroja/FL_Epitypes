...
# Change directory
setwd("/Users/rajesh/Dropbox/Documents/CAN_IMG/01_PMCRC/Projects/00_Projects/01_FLOMICS/00_Methylation_Analysis/04_Summary/450k/modified_InfiniumPurify/Enmix_quantile_with_QCinfo_Harman_All/05_ClusterAIC/")
...
flexmix_clust <- read.table(file = "/Users/rajesh/Dropbox/Documents/CAN_IMG/01_PMCRC/Projects/00_Projects/01_FLOMICS/00_Methylation_Analysis/00_Minfi/db/ClusterAIC/2023-05-06_14_GMM_Cluster_Labels_flexmix_clusters.txt", 
                            sep = "\t", header = TRUE, na.strings = c("","NA")) %>% 
...
Sample_Annotations <- Sample_Annotations %>%
  mutate(ClusterAIC = ifelse(is.na(ClusterAIC), "Not Assigned", ClusterAIC))

table(Sample_Annotations$ClusterAIC)
...
# Step 1: Count occurrences of each ClusterAIC category within each epitype
cluster_aic_data <- Sample_Annotations %>%
  filter(epitypes != "UC_FL") %>%  # Remove epitype = "UC"
  filter(ClusterAIC != "Not Assigned") %>%
  group_by(epitypes, ClusterAIC) %>%
  summarise(Count = n(), .groups = "drop") %>%
  group_by(epitypes) %>% 
  mutate(Proportion = Count / sum(Count),  # Normalize to make proportions sum to 1 within each epitype
         Proportion_Label = paste(Count, " (", scales::percent(Proportion, accuracy = 0.1), ")", sep = ""))  # Create label with count and percentage
...
# Convert factors for proper ordering in plot
cluster_aic_data$ClusterAIC <- factor(cluster_aic_data$ClusterAIC, levels = c("CS", "TT", "GM", "Q", "AR", "Not Assigned"))

# Define custom colors for ClusterAIC groups
cluster_aic_colors <- c(
  "CS" = "#ffa9a3",
  "TT" = "#e7e8b0",
  "GM" = "#addece",
  "Q"  = "#a2cfe0",
...
# Rename epitypes
table(cluster_aic_data$epitypes)

cluster_aic_data$epitypes[cluster_aic_data$epitypes == 'C1_FL'] <- 'iFL'
cluster_aic_data$epitypes[cluster_aic_data$epitypes == 'C2_FL'] <- 'aFL'
cluster_aic_data$epitypes <- factor(cluster_aic_data$epitypes, levels = c("iFL", "aFL")) # reorder

table(cluster_aic_data$epitypes)

# Save plot as PDF
pdf("ClusterAIC.pdf", width = 6.5, height = 6)

ggplot(cluster_aic_data, aes(x = epitypes, y = Proportion, fill = ClusterAIC)) +
  
  # Plot all bars first WITHOUT outline
  geom_bar(data = subset(cluster_aic_data, epitypes == "t_DLBCL"), 
           stat = "identity", position = "stack", width = 0.7) +
  ...
  scale_fill_manual(values = cluster_aic_colors, na.value = "#c9c9c9") +  
  labs(x = "Epitypes",
       y = "Proportion",
       fill = "FL Genetic Subtypes") +
  theme_minimal() +
  theme(
    axis.title = element_text(size = 16, face = "bold"),
